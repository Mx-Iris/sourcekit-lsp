...
final class LocalSwiftTests: XCTestCase {
    ...
    var connection: TestSourceKitServer!
    ...
    func testFixitsAreReturnedFromCodeActions() throws {
        ...
        guard case let .codeActions(codeActions) = response else {
    ...
    func testSyntaxHighlight() throws {
        let url = URL(fileURLWithPath: "/\(UUID())/a.swift")
        let uri = DocumentURI(url)
        ...
        let response = try sk.sendSync(DocumentSemanticTokensRequest(textDocument: TextDocumentIdentifier(uri)))
        print(response!)
        
        if let languageServer = connection.server?.languageServices[.swift]?.first as? SwiftLanguageServer,
            let snapshot = languageServer.documentManager.latestSnapshot(uri) {
