            ...
            for (uri, change) in changedFiles {
                ...
                guard self.documentsReady.contains(uri) else {
                    ...
                    guard let service = workspace.documentService[uri] else {
                        ...
                        log(
                            "No language service for build settings change to non-ready file \(uri)",
                            level: .error
                        )
    ...
    public func filesDependenciesUpdated(_ changedFiles: Set<DocumentURI>) {
        queue.async {
            ...
            let changedFilesAndWorkspace = changedFiles.map {
                (uri: $0, workspace: self.workspaceForDocument(uri: $0))
            }
...
extension SourceKitServer {
    ...
    func initialize(_ req: Request<InitializeRequest>) {
        if case let .dictionary(options) = req.params.initializationOptions {
            if case let .bool(listenToUnitEvents) = options["listenToUnitEvents"] {
                self.options.indexOptions.listenToUnitEvents = listenToUnitEvents
            }
            if case let .dictionary(completionOptions) = options["completion"] {
                if case let .bool(serverSideFiltering) = completionOptions["serverSideFiltering"] {
        ...
        req.reply(InitializeResult(capabilities:
            serverCapabilities(for: req.params.capabilities, registry: capabilityRegistry!)))
    }
    ...
    func registerCapabilities(
        ...
        var watchers = FileRuleDescription.builtinRules.flatMap { $0.fileTypes }.map { fileExtension in
            FileSystemWatcher(globPattern: "**/*.\(fileExtension)", kind: [.create, .delete])
        }
    ...
    private func dynamicallyRegisterCapability(
        ...
        _ = client.send(req, queue: queue) { result in
    ...
    private func openDocument(_ note: DidOpenTextDocumentNotification, workspace: Workspace) {
        ...
        documentToPendingQueue[uri, default: DocumentNotificationRequestQueue()].add(operation: {
            service.openDocument(note)
        })
    }
    ...
    func closeDocument(_ note: Notification<DidCloseTextDocumentNotification>) {
        ...
        closeDocument(note.params, workspace: workspace)
    }
    ...
    func closeDocument(_ note: DidCloseTextDocumentNotification, workspace: Workspace) {
        ...
        documentToPendingQueue[uri]?.cancelAll()
        documentToPendingQueue[uri] = nil
    }
    ...
    func didChangeWorkspaceFolders(_ note: Notification<DidChangeWorkspaceFoldersNotification>) {
        ...
        for docUri in documentManager.openDocuments {
            preChangeWorkspaces[docUri] = workspaceForDocument(uri: docUri)
        }
        if let removed = note.params.event.removed {
            workspaces.removeAll { workspace in
                removed.contains(where: { workspaceFolder in
        ...
        if let added = note.params.event.added {
            ...
            workspaces.append(contentsOf: newWorkspaces)
        }
        ...
        for docUri in documentManager.openDocuments {
            ...
            let newWorkspace = workspaceForDocument(uri: docUri)
            if newWorkspace !== oldWorkspace {
                ...
                if let oldWorkspace = oldWorkspace {
                    closeDocument(DidCloseTextDocumentNotification(
                        textDocument: TextDocumentIdentifier(docUri)
                    ), workspace: oldWorkspace)
                }
                if let newWorkspace = newWorkspace {
                    openDocument(DidOpenTextDocumentNotification(textDocument: TextDocumentItem(
                        uri: docUri,
                        language: snapshot.document.language,
                        version: snapshot.version,
                        text: snapshot.text
    ...
    func executeCommand(_ req: Request<ExecuteCommandRequest>) {
        ...
        fowardExecuteCommand(req, languageService: languageService)
    }
    ...
    func fowardExecuteCommand(
        ...
        let callback = callbackOnQueue(queue) { (result: Result<ExecuteCommandRequest.Response, ResponseError>) in
            req.reply(result)
        }
        let request = Request(
            executeCommand,
    ...
    func codeAction(
        ...
        let callback = callbackOnQueue(queue) { (result: Result<CodeActionRequest.Response, ResponseError>) in
            switch result {
    ...
    func definition(
        ...
        let index = workspaceForDocument(uri: req.params.textDocument.uri)?.index
        let callback = callbackOnQueue(queue) { (result: LSPResult<SymbolInfoRequest.Response>) in

            // If this symbol is a module then generate a textual interface
            if case let .success(symbols) = result, let symbol = symbols.first, symbol.kind == .module, let name = symbol.name {
                ...
                let request = Request(
                    ...
                    clientID: ObjectIdentifier(self),
                    cancellation: req.cancellationToken,
                    reply: { (result: Result<OpenInterfaceRequest.Response, ResponseError>) in
                        switch result {
                        case let .success(interfaceDetails?):
    ...
    func incomingCalls(_ req: Request<CallHierarchyIncomingCallsRequest>) {
        guard let data = extractCallHierarchyItemData(req.params.item.data),
              let index = workspaceForDocument(uri: data.uri)?.index else {
            req.reply([])
            return
        }
    ...
    func prepareTypeHierarchy(
        ...
        let symbolInfo = SymbolInfoRequest(textDocument: req.params.textDocument, position: req.params.position)
        guard let index = workspaceForDocument(uri: req.params.textDocument.uri)?.index else {
            req.reply([])
            return
        }
        let callback = callbackOnQueue(queue) { (result: LSPResult<SymbolInfoRequest.Response>) in
