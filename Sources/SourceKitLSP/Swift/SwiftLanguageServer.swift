...
private extension Range {
    ...
    func overlapsIncludingEmptyRanges(other: Range<Bound>) -> Bool {
        ...
        case (true, false):
            return other.contains(lowerBound)
        case (false, true):
...
public struct SwiftCompileCommand: Equatable {
    ...
    public init?(change: FileBuildSettingsChange) {
        switch change {
        case let .fallback(settings): self.init(settings, isFallback: true)
        case let .modified(settings): self.init(settings, isFallback: false)
...
public final class SwiftLanguageServer: ToolchainLanguageServer {
    ...
    public let queue: DispatchQueue = .init(label: "swift-language-server-queue", qos: .userInitiated)
    ...
    var currentCompletionSession: CodeCompletionSession?
    ...
    func handleDocumentUpdate(uri: DocumentURI) {
        ...
        let compileCommand = commandsByFile[uri]
        ...
        if let dict = try? sourcekitd.sendSync(req) {
...
public extension SwiftLanguageServer {
    ...
    private func reopenDocument(_ snapshot: DocumentSnapshot, _ compileCmd: SwiftCompileCommand?) {
        ...
        let closeReq = SKDRequestDictionary(sourcekitd: sourcekitd)
        closeReq[keys.request] = requests.editor_close
        closeReq[keys.name] = path
        _ = try? sourcekitd.sendSync(closeReq)

        let openReq = SKDRequestDictionary(sourcekitd: sourcekitd)
        ...
        guard let dict = try? sourcekitd.sendSync(openReq) else {
        ...
        updateSyntacticTokens(for: snapshot)
    }
    ...
    func changeDocument(_ note: DidChangeTextDocumentNotification) {
        ...
        queue.async {
            var lastResponse: SKDResponseDictionary?
    ...
    internal func retrieveCodeActions(_ req: Request<CodeActionRequest>, providers: [CodeActionProvider], completion: @escaping CodeActionProviderCompletion) {
        ...
        for i in 0 ..< providers.count {
            queue.async {
                providers[i](req.params) { result in
                    ...
                    guard case let .success(actions) = result else {
                        return
                    }
                    codeActions += actions
                }
    ...
    internal func retrieveRefactorCodeActions(_ params: CodeActionRequest, completion: @escaping CodeActionProviderCompletion) {
    ...
    internal func retrieveQuickFixCodeActions(_ params: CodeActionRequest, completion: @escaping CodeActionProviderCompletion) {
    ...
    internal func applyEdit(label: String, edit: WorkspaceEdit, completion: @escaping (LSPResult<ApplyEditResponse>) -> Void) {
        let req = ApplyEditRequest(label: label, edit: edit)
        let handle = client.send(req, queue: queue) { reply in
            switch reply {
            case let .success(response) where response.applied == false:
                let reason: String
...
extension SwiftLanguageServer: SKDNotificationHandler {
    public func notification(_ notification: SKDResponse) {
        ...
        if let kind: sourcekitd_uid_t = dict[keys.notification],
           kind == values.notification_documentupdate,
...
extension sourcekitd_uid_t {
    func isCommentKind(_ vals: sourcekitd_values) -> Bool {
        switch self {
        case vals.syntaxtype_comment,
             vals.syntaxtype_comment_marker,
             vals.syntaxtype_comment_url:
            return true
        default:
